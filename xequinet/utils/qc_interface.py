...
class TwoBodyBlockPad:
    ...
    def __call__(self, atomsybs: Iterable, feat_ten: torch.Tensor) -> torch.Tensor:
        """
        Args:
    ...
    def __init__(
        ...
        for ele in possible_elements:
            ...
            for ao in basis_set:
                ...
                offset += 2*l + 1
        self.out_repid_mask = out_repid_mask.bool()

    def __call__(self, atomic_numbers: torch.Tensor, edge_index: torch.Tensor) -> Tuple[torch.BoolTensor, torch.BoolTensor]:
        """
        Args:
...
class Mat2GraphLabel:
    ...
    def __call__(self, data, feat_matrice: torch.Tensor, atom_sybs: Iterable = None, edge_index: torch.Tensor = None):
...
class QCMatriceBuilder:
    ...
    def __call__(
        ...
        off_diagnol_mask = to_dense_adj(edge_index, batch=batch_index, max_num_nodes=max_natms).bool()
...
class BuildMatPerMole:
    def __init__(
        ...
        for ele in possible_elements:
            atm_num = ELEMENTS_DICT[ele]
            basis_set = load_basis(basisname, ele)
            for ao in basis_set:
                l = ao[0]
                elem_num_basis[atm_num] += 2*l + 1 
        # register buffer
