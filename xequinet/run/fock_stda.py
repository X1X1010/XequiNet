...
def A_X_build(mf, singlet=True):
    ...
    if singlet == True:
        ...
        def A_X(X: np.ndarray) -> np.ndarray:
            result = 2 * np.einsum(
                "Aia, Ajb, xjb -> xia", q_a_ia, q_b_jb_g_ab_k, X, optimize=True
            )
            for x in range(X.shape[0]):
                result[x] -= np.einsum(
                    "Aij, Aab, jb -> ia", q_a_ij, q_b_ab_g_ab_j, X[x], optimize=True
                )
            return result
        ...
        def A_X(X: np.ndarray) -> np.ndarray:
            result = -1 * np.einsum(
                "Aij, Aab, xjb -> xia", q_a_ij, q_b_ab_g_ab_j, X, optimize=True
            )
            return result
...
def gen_stda_operation(mf, fock_ao=None, singlet=True, wfnsym=None):
    ...
    if fock_ao is None:
        ...
        fock = reduce(
            np.dot, (mo_coeff.conj().T, fock_ao, mo_coeff)
        )  # here replace fock_ao with fock_matrix
...
def cal_orbital_and_energies(
    ovlp: np.ndarray, fock: np.ndarray
    ...
    for imol, data in enumerate(dataset, start=1):
        with torch.no_grad():
            ...
            data.edge_index = radius_graph(
                data.pos, r=config.cutoff, max_num_neighbors=config.max_edges
            )
