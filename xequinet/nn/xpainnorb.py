...
class XMatTrans(nn.Module):
    ...
    def __init__(
        ...
        if self.irreps_in == self.irreps_hidden:
            ...
            self.pretrans = True
            self.node_pre_trans = o3.Linear(self.irreps_in, self.irreps_hidden, biases=False)
        self.node_self_layer = SelfLayer(self.irreps_hidden, self.irreps_block, actfn)
        self.node_pair_layer = PairLayer(self.irreps_hidden, self.irreps_block, edge_dim, actfn)
