...
class MatEmbedding(nn.Module):
    ...
    def __init__(
        ...
        self.node_irreps = o3.Irreps(
            [(node_channels, (l, (-1) ** l)) for l in range(max_l + 1)]
        )
        ...
        self.rsh_conv = o3.SphericalHarmonics(
            self.irreps_rsh, normalize=True, normalization="component"
        )
...
class NodewiseInteraction(nn.Module):
    ...
    def __init__(
        ...
        self.mlp_edge_scalar = nn.Sequential(
            nn.Linear(
                self.irreps_node_in.num_irreps + self.node_channels, 32, bias=True
            ),
            self.activation,
        ...
        if self.not_first_layer:  # use gate with MLP
            self.lin_node0 = o3.Linear(
                self.irreps_node_in, self.irreps_node_in, biases=True
            )
            self.lin_node1 = o3.Linear(
                self.irreps_node_in, self.irreps_node_in, biases=True
            )
            self.gate = Gate(self.irreps_node_in, refine=True)
        self.lin_node2 = o3.Linear(
            self.irreps_node_out, self.irreps_node_out, biases=True
        )
    ...
    def forward(
        ...
        if self.not_first_layer:
            ...
            s0 = self.inner_dot(pre_x[edge_index[0]], pre_x[edge_index[1]])[
                :, self.num_scalar :
            ]
            ...
            s0 = self.inner_dot(x[edge_index[0]], x[edge_index[1]])[
                :, self.num_scalar :
            ]
...
class MatrixOut(nn.Module):
    ...
    def forward(
        ...
        node_embed_pair = torch.cat(
            [node_embed[edge_index_full[0]], node_embed[edge_index_full[1]]], dim=-1
        )
        ...
        offdiagnol_block_T = torch.index_select(
            offdiagnol_block.transpose(-1, -2), dim=0, index=src_indices
        )
...
class NodeInterWithEle(NodewiseInteraction):
    ...
    def forward(
        ...
        x_electron = self.charge_ebd(x_scalar, charge, batch) + self.spin_ebd(
            x_scalar, spin, batch
        )
