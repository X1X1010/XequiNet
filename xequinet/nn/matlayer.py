...
class PairLayer(nn.Module):
    ...
    def __init__(
        ...
        self.irreps_tp_out, instruct = get_feasible_tp(
            self.irreps_in, self.irreps_in, self.irreps_hidden, "uuu",
        )
        ...
        self.linear_node_post = o3.Linear(self.irreps_tp_out, self.irreps_hidden, biases=False)
    ...
    def forward(
        ...
        node_feat = x
        s0 = self.inner_dot(node_feat[edge_index[0], :], node_feat[edge_index[1], :])[:, self.num_scalar :]
        s0 = torch.cat([
...
class Expansion(nn.Module):
    ...
    def __init__(
        ...
        self.instructions, self.w3j_matrices = self.get_expansion_path(irreps_block, irreps_out, irreps_out)
            ...
            for j, (mul_out1, ir_out1) in enumerate(irreps_out1):
                for k, (mul_out2, ir_out2) in enumerate(irreps_out2):
                    if ir_in in ir_out1 * ir_out2:
                        instructions.append((i, j, k, True, 1.0, (mul_in, mul_out1, mul_out2)))
                        w3j_ijk = nn.Parameter(o3.wigner_3j(ir_out1.l, ir_out2.l, ir_in.l), requires_grad=False)
    ...
    def forward(self, x_in: torch.Tensor, node_embed: torch.Tensor) -> torch.Tensor:
        ...
        for ins, w3j in zip(self.instructions, self.w3j_matrices):
            ...
            x1 = x_in_s[ins[0]]
            if ins[3] is True:
                w = weight[:, flat_weight_index : flat_weight_index + prod(ins[-1])].view(-1, *ins[-1])
                result = torch.einsum("bwuv, bwk -> buvk", w, x1)
                if ins[0] == 0:
                    b = bias[:, bias_index : bias_index + prod(ins[-1][1:])].view(-1, *ins[-1][1:])
        ...
        for i in range(len(self.irreps_out)):
            ...
            for j in range(len(self.irreps_out)):
                if (i, j) not in outputs.keys():
                    # mul_ir_out.dim = mul_ir_out.mul * (2 * mul_ir_out.ir.dim + 1)
                    out_dimi = self.irreps_out[i][0] * (2 * self.irreps_out[i][1][0] + 1)
                    out_dimj = self.irreps_out[j][0] * (2 * self.irreps_out[j][1][0] + 1)
                    blocks.append(
                        torch.zeros(
                            (x_in.shape[0], out_dimi, out_dimj), device=x_in.device, dtype=x_in.dtype,
                        )
                    )
                else:
