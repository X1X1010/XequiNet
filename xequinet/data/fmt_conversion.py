...
def datapoint_from_ase(atoms: ase.Atoms, dtype: torch.dtype = None) -> XequiData:
    ...
    return XequiData(
        ...
        cell=torch.from_numpy(cell).view(1, 3, 3).to(dtype) if cell is not None else None,
    )
...
def datapoint_to_pyscf(datapoint: XequiData) -> gto.MoleBase:
    ...
    if hasattr(datapoint, keys.PBC) and datapoint.pbc is not None and datapoint.pbc.any():
        cell = datapoint.cell.view(3, 3).numpy()
        mole = pbc.gto.Cell(
