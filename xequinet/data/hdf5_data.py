...
class BaseH5Dataset:
    def _init_base(
        ...
        self._max_edges = (
            config.max_edges
        )  # maximum edge numbers for each single system
        self.transform = transform  # transform function, e.g., subtract atomic energy
        self.pre_transform = (
            pre_transform  # pre-transform function, e.g., unit conversion
        )
    ...
    def process_h5(self, f_h5: h5py.File) -> Iterable[Data]:
        ...
        for mol_name in f_h5[self._mode].keys():
            ...
            if "coordinates_A" in mol_grp.keys():
                coords = torch.from_numpy(mol_grp["coordinates_A"][()]).to(
                    torch.get_default_dtype()
                )
                coords *= unit_conversion("Angstrom", self._len_unit)
            elif "coordinates_Bohr" in mol_grp.keys():
                coords = torch.from_numpy(mol_grp["coordinates_Bohr"][()]).to(
                    torch.get_default_dtype()
                )
            ...
            if self._ele:
                charge = (
                    float(mol_grp["charge"][()]) if "charge" in mol_grp.keys() else 0.0
                )
                spin = (
                    float(mol_grp["multiplicity"][()] - 1)
                    if "multiplicity" in mol_grp.keys()
                    else 0.0
                )
            ...
            for icfm, coord in enumerate(coords):
                edge_index = radius_graph(
                    coord, r=self._cutoff, max_num_neighbors=self._max_edges
                )
                data = mol_data.clone()
                data.pos = coord
    ...
    def process_pbch5(self, f_h5: h5py.File) -> Iterable[Data]:
        ...
        for pbc_name in f_h5[self._mode].keys():
            ...
            if self._ele:
                charge = (
                    float(pbc_grp["charge"][()]) if "charge" in pbc_grp.keys() else 0.0
                )
                spin = (
                    float(pbc_grp["multiplicity"][()] - 1)
                    if "multiplicity" in pbc_grp.keys()
                    else 0.0
                )
                charge = torch.Tensor([charge]).to(torch.get_default_dtype())
                spin = torch.Tensor([spin]).to(torch.get_default_dtype())
                pbc_data.charge = charge
                pbc_data.spin = spin
            # loop over conformations
            for icfm, coord in enumerate(coords):
    ...
    def process_matrixh5(self, f_h5: h5py.File) -> Iterable[Data]:
        ...
        for mol_name in f_h5[self._mode].keys():
            ...
            for icfm, coord in enumerate(coords):
                ...
                data.pos = coord
                data.edge_index = edge_index
                matrix = torch.from_numpy(mol_grp[self._prop_dict["y"]][()][icfm]).to(
                    torch.get_default_dtype()
                )
                ...
                node_blocks, edge_blocks = mat_toolkit.get_padded_blocks(
                    at_no, matrix, mat_edge_index
                )
                node_mask, edge_mask = mat_toolkit.get_mask(at_no, mat_edge_index)
                data.node_label = node_blocks
                data.edge_label = edge_blocks
...
class H5Dataset(BaseH5Dataset, Dataset):
    ...
    def __init__(
        ...
        super()._init_base(
            config, mode=mode, transform=transform, pre_transform=pre_transform
        )
        super().__init__()
...
class H5MemDataset(BaseH5Dataset, InMemoryDataset):
    ...
    def __init__(
        ...
        self._data_name: str = (
            f"{self._raw_files[0].split('.')[0]}" if data_name is None else data_name
        )
        self._processed_file = f"{self._data_name}_{self._mode}.pt"

        super().__init__(
            root, transform=self.transform, pre_transform=self.pre_transform
        )
    ...
    def process(self) -> None:
        ...
        for raw_path in self.raw_paths:
            ...
            for data in data_iter:
                ...
                os.makedirs(
                    os.path.join(data_dir, f"{idx // 10000:04d}"), exist_ok=True
                )
                torch.save(
                    data, os.path.join(data_dir, f"{idx // 10000:04d}", f"{idx:08d}.pt")
                )
                idx += 1
            # close hdf5 file
            if self._mem_process:
    ...
    def len(self) -> int:
        if self._num_data is None:
            ...
            max_dir = os.path.join(
                data_dir, max([d for d in os.listdir(data_dir) if d.isdigit()])
            )
...
def data_unit_transform(
    ...
    if hasattr(new_data, "force"):
        ...
        if hasattr(new_data, "base_force"):
            new_data.base_force *= unit_conversion(
                bforce_unit, f"{prop_unit}/{len_unit}"
            )
    # for matrix
...
def create_dataset(
    config: NetConfig, mode: str = "train", local_rank: int = None
) -> Dataset:
    with distributed_zero_first(local_rank):
        # set transform function
        pre_transform = lambda data: data_unit_transform(
            data=data,
            y_unit=config.label_unit,
            by_unit=config.blabel_unit,
            force_unit=config.force_unit,
            bforce_unit=config.bforce_unit,
        )
        ...
        if config.dataset_type == "normal":
            dataset = H5Dataset(
                config, mode=mode, pre_transform=pre_transform, transform=transform
            )
        elif config.dataset_type == "memory":
            dataset = H5MemDataset(
                config, mode=mode, pre_transform=pre_transform, transform=transform
            )
        elif config.dataset_type == "disk":
            dataset = H5DiskDataset(
                config, mode=mode, pre_transform=pre_transform, transform=transform
            )
        else:
